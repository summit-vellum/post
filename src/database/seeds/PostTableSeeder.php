    	...
    	$vellumArticles = $articles->orderBy('article_id')->chunk($itemsPerBatch, function($articles){
    		...
    		foreach ($articles as $article) {
    			...
    			$migratedPost->create([
    				...
    				'created_at'=> ($article->article_original_date_published != 0) ? date('Y-m-d H:m:i', $article->article_original_date_published) : NULL,
    				'updated_at'=> ($article->article_date_modified != 0) ? date('Y-m-d H:m:i', $article->article_date_modified) : NULL
    			]);
